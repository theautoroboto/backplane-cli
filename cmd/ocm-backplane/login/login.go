...
func runLogin(cmd *cobra.Command, argv []string) (err error) {
	...
	logger.Debugf("##############----bpConfig.ProxyURL----: %v", bpConfig.ProxyURL)
	logger.Debugf("##############----bpConfig.DisplayClusterInfo----: %v", bpConfig.DisplayClusterInfo)
	logger.Debugf("##############----bpConfig.ProdEnvName----: %v", bpConfig.ProdEnvName)
